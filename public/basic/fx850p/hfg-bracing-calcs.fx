1 'HFG BRACING CALCS
2 CLS:CLEAR
10 DIM C(2),I(2),O(2),W(2),LA(2),LB(2),LC(2),PLUS(2),SJ(2),JH(2),REV$(2),O1(2),BRI(2),TRAV(2)
20 ANGLE 0:SET N
...
1064 IF N=2 AND C(2)<0 THEN C(2)=C(1):I(2)=I(1):O(2)=O(1):W(2)=W(1):BRI(2)=BRI(1):SYM=1:GOTO 1066
1065 GOTO 1110
1066 LA(2)=LA(1):LB(2)=LB(1):LC(2)=LC(1):O1(2)=O1(1):GOTO 1110
1079 CLS
...
1092 IF I(N)<=37.5 AND W(N)=1 THEN PRINT "REV PREP";CHR$(13);
...
3002 JH(1)=LB(1)*2+O1(1)/SIN(I(1)):IF I(1)<=37.5 AND W(1)=1 THEN JH(1)=JH(1)+LC(1)
...
3075 LGTH=INT(LGTH+.5):WGHT=INT(WGHT+.5):SA=INT(SA*1000+.5)/1000:TRAV(1)=INT(TRAV(1)+.5):TRAV(2)=INT(TRAV(2)+.5)
3300 GOTO 9000
3390 '*******CALCS INC IN LGTH
3400 ROT=1:COUNT=1
3405 IF SYM=1 AND N=2 THEN PLUS(2)=PLUS(1):TRAV(2)=TRAV(1):GOTO 3670
3410 IF I(N)=90 THEN TRAV(N)=C(N)-SQR(C(N)^2-(O(N)+BRI(N))^2):PLUS(N)=TRAV(N):GOTO 3670
3420 IF C(N)=0 AND W(N)=0 THEN PLUS(N)=LC(N):GOTO 3670
3430 IF C(N)=0 AND W(N)=1 THEN GOTO 3670
3500 A1=(BRI(N)*SIN(ROT))+O(N)
...
9075 IF I(1)=90 THEN PRINT "TRAVEL=";TRAV(1)
